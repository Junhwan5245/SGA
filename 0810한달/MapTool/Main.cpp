...
void Main::Update()
{
	...
	if (isLayer1) //레이어 2
	{
		...
		//if (ImGui::SliderInt2("TileSize", (int*)&tileSize, 1, 100))
		//{
		//	floor->ResizeTile(tileSize);
		//}

		////TileScale
		//ImGui::SliderFloat2("TileScale", (float*)&floor->scale, 1.0f, 200.0f);
		//if (ImGui::InputFloat("TileScale", (float*)&floor->scale.x, 1.0f, 200.0f))
		//{
		//	floor->scale.y = floor->scale.x;
		//}

		////TilePos
		Vector2 pos = floor->GetWorldPos();
		if (ImGui::SliderFloat2("TilePos", (float*)&pos, -1000.0f, 1000.0f))
		{
			floor->SetWorldPos(pos);
		}
		...
		for (int i = 0; i < 4; i++)
		{
				...
				".jpg,.png,.bmp,.dds,.tga", "../Contents/Images"))
			{
				...
				size_t tok = path.find("/Images/") + 8;
				path = path.substr(tok, path.length());
				SafeDelete(floor->tileImages[i]);
				wstring wImgFile = L"";
				wImgFile.assign(path.begin(), path.end());
				floor->tileImages[i] = new ObImage(wImgFile);
				break;
			}
			if (i < 3)
			{
		...
		if (ImGui::InputInt("ImgIdx", &brushImgIdx))
		{
			...
			if (not floor->tileImages[brushImgIdx])
			{
				brushImgIdx = 0;
			}
		}
		//maxFrame
		ImGui::InputInt2("maxFrame", (int*)&floor->tileImages[brushImgIdx]->maxFrame);



		Int2 MF = floor->tileImages[brushImgIdx]->maxFrame;
		ImVec2 size;
		size.x = 300.0f / (float)MF.x;
		size.y = 300.0f / (float)MF.y;
		ImVec2 LT, RB;
		...
		for (UINT i = 0; i < MF.y; i++)
		{
			for (UINT j = 0; j < MF.x; j++)
			{
				...
				ImGui::PushID(index);
				if (ImGui::ImageButton((void*)floor->tileImages[brushImgIdx]->GetSRV()
					, size, LT, RB))
				{
					brushFrame.x = j;
					brushFrame.y = i;
		...
		if (INPUT->KeyPress(VK_LBUTTON))
		{
			Int2 Idx;
			//?
			if (floor->WorldPosToTileIdx(INPUT->GetWorldMousePos(), Idx))
			{
				floor->SetTile(Idx, brushFrame, brushImgIdx, brushState, brushColor);
			}
