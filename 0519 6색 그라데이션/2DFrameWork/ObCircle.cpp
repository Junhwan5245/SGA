...
void ObCircle::CreateStaticMember()
{
    ...
    for (UINT i = 0; i < 360; i++)
    {
       ...
       /* Vertex[i * 3 + 2].color = Color(RANDOM->Float(), RANDOM->Float(), RANDOM->Float(), 1.0f);*/
        ...
        //    float color1 = i / 60.0f;
        //    float color2 = (i + 1.0f) / 60.0f;

        //    Vertex[i * 3 + 1].color = Color(1.0f, color1, 0.0f, 1.0f);
        //    Vertex[i * 3 + 2].color = Color(1.0f , color2, 0.0f, 1.0f);
        //}
        if (i < 60)
        {
        ...
        else if (i < 180)
        {
            ...
            Vertex[i * 3].color.x = 0.0f;
            Vertex[i * 3].color.y = 1.0f;
            Vertex[i * 3].color.z = b;
           
            Vertex[i * 3 + 1].color.x = 0.0f;
            Vertex[i * 3 + 1].color.y = 1.0f;
            Vertex[i * 3 + 1].color.z = b;
           
            Vertex[i * 3 + 2].color.x = 0.0f;
            Vertex[i * 3 + 2].color.y = 1.0f;
            Vertex[i * 3 + 2].color.z = b;
        }
        else if (i < 240)
        {
            int n = i-180;
            float b = (60.0f - n) / 60.0f;
            //float b = n / 120.0f;
            //0 ~ 120
            //100 010

            Vertex[i * 3].color.x = 0.0f;
            Vertex[i * 3].color.y = b;
            Vertex[i * 3].color.z = 1.0f;

            Vertex[i * 3 + 1].color.x = 0.0f;
            Vertex[i * 3 + 1].color.y = b;
            Vertex[i * 3 + 1].color.z = 1.0f;

            Vertex[i * 3 + 2].color.x = 0.0f;
            Vertex[i * 3 + 2].color.y = b;
            Vertex[i * 3 + 2].color.z = 1.0f;
        }
        else if (i < 300)
        {
            int n = i - 240.0f;
            ...
            Vertex[i * 3].color.x = r;
            
            Vertex[i * 3].color.z = 1.0f;

            Vertex[i * 3 + 1].color.x = r;
            Vertex[i * 3 + 1].color.z = 1.0f;
            

            Vertex[i * 3 + 2].color.x = r;
        ...
        else
        {
            ...
            Vertex[i * 3].color.z = r;

            Vertex[i * 3 + 1].color.x = 1.0f;
            
            Vertex[i * 3 + 1].color.z = r;

            Vertex[i * 3 + 2].color.x = 1.0f;
            
            Vertex[i * 3 + 2].color.z = r;
        }
